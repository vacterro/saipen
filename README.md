...
**Fast keys.** A shortcut is the entire message, never a prefix. `cc` keeps an active Goal Mode run moving, `sss` reports status without touching code, and `ss` checkpoints and stops. [See the full 14-key map](saipen/RFC.md#110-command-surface). Cyrillic twins work too: `сс`, `ссс`, `аа`, `ее`, `еее`, `рр`.
...
**v7.192.0** | [Spec](SPEC.md) | [Guide](GUIDE.md) | [RFC](saipen/RFC.md) | [Style](saipen/STYLE.md) | [UI](saipen/UI.md) | [Conformance](saipen/CONFORMANCE.md) | MIT
