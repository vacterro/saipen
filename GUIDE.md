...
## How it works
1. You run the `inject.ps1` script. It creates a `.asp/` directory.
2. The agent reads `.asp/STATE.md` and `.asp/BOARD.md`.
3. The agent negotiates its capabilities against the protocol's demands.
4. The agent goes to work, saving its progress to `.asp/` atomically.
5. You can type `asp validate` at any time to ensure the agent hasn't corrupted its memory.
...
| Command | Action |
|---|---|
| `asp continue` | Wake up the agent and execute the `next_action`. |
| `asp stop` | Force the agent to checkpoint and handoff. |
| `asp status` | Report current state without changing anything. |
| `asp validate` | Run the conformance checks. |
