...
**Fast keys:** `cc` continues the project context to convergence, `sss` checks status without touching code, and `st` checkpoints and stops. [Full 19-key map](saipen/RFC.md#110-command-surface); Cyrillic twins `сс`, `ссс`, `аа`, `ее`, `еее`, `рр` work too. `ff` → `focus`; `xx` → `cut`; `vv` → `build`; `zz` → `undo`.
