...
**ショートカット:** ショートカットはメッセージ全体であり、プレフィックスではありません。`cc` は有効な Goal Mode の実行を継続し、`sss` はコードに触れずに状況を報告し、`ss` はチェックポイントを保存して停止します。[全13キーの一覧](saipen/RFC.md#110-command-surface)。キリル文字の同形キーも使えます: `сс`, `ссс`, `аа`, `ее`, `еее`, `рр`。
...
**v7.176.0** | [仕様](SPEC.md) | [ガイド](GUIDE.md) | [RFC](saipen/RFC.md) | [スタイル](saipen/STYLE.md) | [UI](saipen/UI.md) | [適合性](saipen/CONFORMANCE.md) | MIT
...
**プロジェクトの状態はモデルのメモリに勝る。** メモリはモデルの頭の中ではなく、プロジェクト内に存在します。`プロジェクト -> メモリ -> LLM` が `プロジェクト -> SAIPEN 状態 -> LLM` に変わります。
...
全体は16コマンド。完全な詳細は [RFC § 1.10](saipen/RFC.md#110-command-surface)。

| コマンド | 機能 |
|---|---|
| `/saipen set` | プロジェクトを採用する |
| `/saipen continue` | 中断した場所から正確に再開する |
| `/saipen plan` | リクエストまたは生のバックログをチケットに変換する |
| `/saipen goal <text>` | 新しい目標に対する自律的なウェーブ実行 |
| `/saipen hunt` | 今すぐ欠陥/改善スイープを強制する |
| `/saipen ship` | バージョンバンプ、変更ログ、タグ、プッシュ |
| `/saipen clean` | リポジトリのスクラブ |
| `/saipen validate` | 適合性チェック |
| `/saipen markhunt` | 無制限のドライ監査、記録のみ |
| `/saipen translate` | 分離された翻訳ファクトリー |
| `/saipen prepare` | 引き継ぎのための作業のパッケージ化 |
| `/saipen collect` | 準備完了パッケージの統合 |
| `/saipen status` | 読み取り専用レポート |
| `/saipen stop` | チェックポイントと停止 |

<sub>`saipen init` と `saipen sub` で16個が完成します。どちらも日常的に入力されるものではなく、プロトコルによって呼び出されます。</sub>

**パッケージ用キー:** `ee`/`qq` は翻訳/Wikiの完全なパッケージを準備するだけで、統合しません。`eee`/`qqq` は準備完了のパッケージのみを受け取り、統合・検証・レビュー後にプッシュします。
