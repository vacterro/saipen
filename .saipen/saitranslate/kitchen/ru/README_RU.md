...
**Быстрые клавиши:** `cc` ведёт проект к схождению (продолжает активную цель, если она задана), `sss` показывает статус без правок кода, а `ss` сохраняет чекпоинт и останавливается. [Полная карта из 15 клавиш](saipen/RFC.md#110-command-surface). Кириллические близнецы тоже работают: `сс`, `ссс`, `аа`, `ее`, `еее`, `рр`.
...
**v7.223.11** | [Спецификация](SPEC.md) | [Руководство](GUIDE.md) | [RFC](saipen/RFC.md) | [Стиль](saipen/STYLE.md) | [Интерфейс](saipen/UI.md) | [Соответствие](saipen/CONFORMANCE.md) | plain markdown | zero deps | MIT
