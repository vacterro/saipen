...
Käivita [SAIPEN Lakmustest](SPEC.md#the-saipen-litmus-test) oma idee peal:
...
4. Käivita kanooniline validaator enne PR avamist:
   ```bash
   python tools/validate.py
   ```
   See on see, mis loeb -- see kontrollib `STATE.md` vastavust `extensions/schemas/state.schema.json`-ile, käbib `LOG.md` sündmuste graafi läbi tihendatud segmentide ja kontrollib käitusaja manifesti ning injektori ühendusi. `tests/validate.sh` / `validate.ps1` on **külmutatud portatiivne põrand** Pythonita hostidele (nad ütlevad seda ise iga faili alguses): vähem kontrolle, hoitakse ainult selleks, et Pythonita hostil poleks üldse midagi. Käivita neid ka, kui su muudatus puudutab neid, aga mitte *selle asemel* -- põranda läbimine tõestab palju vähem, kui paistab. CI käivitab kanoonilise validaatori igal push-il ja PR-il sõltumata (`.github/workflows/validate.yml`).
...
- SAIPEN-i muutmine hajutatud konsensussüsteemiks. Vaata `SPEC.md` Concurrency & Distribution Boundaries jaotist.
