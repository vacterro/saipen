...
4. **Transition**: If fixed or passed, transition `STATE.phase` to `SCOUT` (if workable tickets exist), `PLAN` (if unrefined tasks exist), `DONE` (if there are no `TODO` tickets remaining, which will trigger Autonomous Evolution per RFC § 2.1), or `BLOCKED` (if all remaining tickets are unworkable).
