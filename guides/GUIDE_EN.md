...
**Fast keys:** `cc` keeps active Goal Mode moving, `sss` checks status without touching code, and `ss` hits the brake after checkpointing. [Full 14-key map](../saipen/RFC.md#110-command-surface); Cyrillic twins `сс`, `ссс`, `аа`, `ее`, `еее`, `рр` work too.
