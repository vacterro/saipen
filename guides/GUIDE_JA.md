...
**ショートカット:** `cc` はコンテキストを収束まで継続し（実行中の目標があればそれを再開）、`sss` はコードを変更せず状態を表示し、`st` はチェックポイントを保存して停止する。[全19キーの一覧](../saipen/RFC.md#110-command-surface)。キリル文字の `сс`、`ссс`、`аа`、`ее`、`еее`、`рр` も使える。 `ff` → `focus`; `xx` → `cut`; `vv` → `build`; `zz` → `undo`.
