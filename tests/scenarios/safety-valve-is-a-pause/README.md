...
`goal_mode: false` on a trip -- while § 1.10 recognizes bare `saipen goal`
ONLY while `goal_mode: true`. Tripping the valve therefore made the one
...
So: `goal_mode` stays `true` through a trip. What prevents a restart from
walking straight past the valve is the counters, not the flag --
`goal_mode: true` with `goal_waves >= 3` or `goal_tickets >= 20` IS the
