...
def run_tests():
    ...
    verify_py = root / "tools" / "verify_handoff_archive.py"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Actually T-1013 says: "open/capture members through one containment/type-stable read path and fail closed if inode/type/target changes".
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r = _run([sys.executable, str(saipen_py), "--project-root", str(project), "userperson", "add", "my key is sk-1234567890123456789012345678901234567890"], project)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        project = tdp / "project"
        project.mkdir()
        r = _run([sys.executable, str(build_py), "out.zip", "--project-root", str(project)], project)
        assert r.returncode != 0
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r = _run([sys.executable, str(build_py), str(out_zip), "--project-root", str(project)], project)
        assert r.returncode != 0
        assert "FAIL: destination is a canonical protected path" in r.stdout or "destination already exists" in r.stdout
