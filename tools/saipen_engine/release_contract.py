...
def release_metadata_paths(root: Path) -> list[Path]:
    ...
    mechanically mirrored locale README under the translation kitchen.

    Both the release executor and the validator use this exact set. All
    returned paths are repository-relative, so callers can feed them
